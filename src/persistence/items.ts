...
function parseAttributes(json: string | null): Record<string, string> | undefined {
  ...
  try {
    ...
    // A plain object only — an array (also typeof 'object') would iterate as
    // numeric indices, so reject it as corrupt.
    if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
      return parsed as Record<string, string>;
    }
  } catch {
    // A corrupt blob is treated as absent rather than failing the read.
  }
  return undefined;
}
...
export class ItemRepo {
   ...
   * `last_seen`. A re-sight also clears any de-listing flags (`gone_count`,
   * `delisted_at`) — the item is plainly present again. Re-listing detection
   * reads those flags BEFORE the upsert (see the cycle), so clearing here is safe.
   */
      ...
      .prepare(
        `INSERT INTO items
           (monitor_id, item_id, in_stock, last_price, currency, first_seen, last_seen,
            title, url, image_url, location, seller_private, posted_at, description,
            attributes_json, gone_count, delisted_at)
         VALUES
           (@monitorId, @itemId, @inStock, @price, @currency, @now, @now,
            @title, @url, @imageUrl, @location, @sellerPrivate, @postedAt, @description,
            @attributesJson, 0, NULL)
