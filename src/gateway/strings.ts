...
export interface Catalog {
  ...
  /** Extra /stats lines (recency, seller split) — only chat-wide-meaningful counts. */
  stats_extra: (p: { new24: number; new7: number; priv: number; comp: number }) => string;
  ...
  /** One best-deal line (tagged with its watch — comps are per-watch). */
  best_deals_line: (p: { rank: number; watch: string; title: string; price: string; discount: number; n: number; url: string }) => string;
...
const ro: Catalog = {
  ...
  stats_extra: ({ new24, new7, priv, comp }) =>
    [
  ...
  best_deals_line: ({ rank, watch, title, price, discount, n, url }) =>
    `${rank}. [${watch}] ${title} — ${price} · −${discount}% sub medie (din ${n})${url ? `\n${url}` : ''}`,
...
const en: Catalog = {
  ...
  best_deals_line: ({ rank, watch, title, price, discount, n, url }) =>
    `${rank}. [${watch}] ${title} — ${price} · −${discount}% below median (of ${n})${url ? `\n${url}` : ''}`,
  browse_filtered_notice: ({ total, breakdown }) =>
...
const de: Catalog = {
  ...
  best_deals_line: ({ rank, watch, title, price, discount, n, url }) =>
    `${rank}. [${watch}] ${title} — ${price} · −${discount}% unter dem Median (von ${n})${url ? `\n${url}` : ''}`,
  browse_filtered_notice: ({ total, breakdown }) =>
...
const it: Catalog = {
  ...
  best_deals_line: ({ rank, watch, title, price, discount, n, url }) =>
    `${rank}. [${watch}] ${title} — ${price} · −${discount}% sotto la mediana (su ${n})${url ? `\n${url}` : ''}`,
  browse_filtered_notice: ({ total, breakdown }) =>
...
const es: Catalog = {
  ...
  best_deals_line: ({ rank, watch, title, price, discount, n, url }) =>
    `${rank}. [${watch}] ${title} — ${price} · −${discount}% bajo la mediana (de ${n})${url ? `\n${url}` : ''}`,
  browse_filtered_notice: ({ total, breakdown }) =>
...
const fr: Catalog = {
  ...
  best_deals_line: ({ rank, watch, title, price, discount, n, url }) =>
    `${rank}. [${watch}] ${title} — ${price} · −${discount}% sous la médiane (sur ${n})${url ? `\n${url}` : ''}`,
  browse_filtered_notice: ({ total, breakdown }) =>
